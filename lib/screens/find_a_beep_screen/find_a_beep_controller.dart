...
class FindABeepController extends GetxController {
      ...
      Marker(
        ...
        onTap: () {
          mapController!.animateCamera(CameraUpdate.newLatLngZoom(
              LatLng(Constants.latitude, Constants.longitude), 16));
        },
  ...
  /// ---- Marker Click ------------>>>
  void markerClick({MachinesResponseModel? model, int i = 0}) async {
    await mapController!.animateCamera(
      CameraUpdate.newLatLngZoom(
          LatLng(double.parse(model!.data![i].lat!),
              double.parse(model.data![i].long!)),
          16),
    );
    for (int i = 0; i < model.data!.length; i++) {
      ...
      markers.add(
        Marker(
          ...
          position: LatLng(double.parse(model.data![i].lat!),
              double.parse(model.data![i].long!)),
          onTap: () async {
            selected = i;
          ...
          icon: BitmapDescriptor.fromBytes(selected == i
              ? await getBytesFromAssets(ImagePath.selectMarker, 190)
              : await getBytesFromAssets(ImagePath.marker, 160)),
        ),
      );
    }
  ...
  void autoCompleteSearch(String value) async {
    if (value.isNotEmpty) {
      var result = await googlePlace!.autocomplete
          .get(value, components: [Component("country", "US")]);
